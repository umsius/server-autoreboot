...
class TelegramNotifier:
    ...
    async def clear_chat(self):
        """Удалить все предыдущие сообщения бота в чате"""
        if not self.bot:
            return
        
        for admin_id in self.admin_ids:
            try:
                for server_name, msg_id in self.status_messages.items():
                    try:
                        await self.bot.delete_message(chat_id=admin_id, message_id=msg_id)
                        logger.info(f"Deleted status message {msg_id} for {server_name}")
                    except TelegramError:
                        pass
                
                for msg_id in self.broadcast_message_ids:
                    try:
                        await self.bot.delete_message(chat_id=admin_id, message_id=msg_id)
                        logger.info(f"Deleted broadcast message {msg_id}")
                    except TelegramError:
                        pass
                
                self.status_messages.clear()
                self.broadcast_message_ids.clear()
                
            except TelegramError as e:
                logger.error(f"Failed to clear chat for {admin_id}: {e}")
...
_Перезагрузка выполнена через API Yandex Cloud_"""
    ...
    async def send_broadcast_message(self, message: str, save_for_cleanup: bool = True):
        ...
        if save_for_cleanup:
            self.broadcast_message_ids.extend(msg_ids)
...
async def restart_single_vm(server_config: dict, notifier: TelegramNotifier):
    ...
    if not server_config['folder_id']:
        ...
        await notifier.send_broadcast_message(f"❌ Ошибка: Не указан folder_id для сервера {server_name}", save_for_cleanup=False)
        return False
...
async def restart_all_vms():
    """Перезагрузить все ВМ по очереди с паузой 10 секунд без уведомления"""
    notifier = TelegramNotifier()
    
    # Очищаем чат от предыдущих сообщений
    await notifier.clear_chat()
    ...
    for i, server in enumerate(active_servers, 1):
        ...
        # Пауза 10 секунд между серверами без уведомления
...
📅 Время завершения: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"""
    ...
    # Очищаем все сообщения бота перед отправкой итога
    await notifier.clear_chat()
    
    # Отправляем только итоговое сообщение
    await notifier.send_broadcast_message(summary, save_for_cleanup=False)
